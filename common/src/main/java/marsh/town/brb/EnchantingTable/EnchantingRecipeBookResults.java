...
public class EnchantingRecipeBookResults {
    private List<EnchantingResult> recipeCollection;
    public final List<EnchantingAnimatedResultButton> resultButtons = Lists.newArrayListWithCapacity(20);
    private int pageCount;
    private int currentPage;
    private StateSwitchingButton nextPageButton;
    private StateSwitchingButton prevPageButton;
    private Minecraft client;
    private EnchantingAnimatedResultButton hoveredResultButton;
    private EnchantingResult lastClickedRecipe;
    RecipeBookGroup group;
    private EnchantmentMenu brewingStandScreenHandler;

    public EnchantingRecipeBookResults() {
        for(int i = 0; i < 20; ++i) {
            this.resultButtons.add(new EnchantingAnimatedResultButton());
        }

    }

    public void initialize(Minecraft client, int parentLeft, int parentTop, EnchantmentMenu brewingStandScreenHandler) {
        this.client = client;
    ...
    public void setResults(List<EnchantingResult> recipeCollection, boolean resetCurrentPage, RecipeBookGroup group) {
    ...
    private void refreshResultButtons() {
        ...
        for(int j = 0; j < this.resultButtons.size(); ++j) {
            EnchantingAnimatedResultButton animatedResultButton = this.resultButtons.get(j);
            if (i + j < this.recipeCollection.size()) {
                EnchantingResult output = this.recipeCollection.get(i + j);
                animatedResultButton.showEnchantment(output, group, brewingStandScreenHandler);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (this.nextPageButton.mouseClicked(mouseX, mouseY, button)) {
            ...
            Iterator<EnchantingAnimatedResultButton> var10 = this.resultButtons.iterator();

            EnchantingAnimatedResultButton animatedResultButton;
            do {
                if (!var10.hasNext()) {
                    return false;
                }
    ...
    public EnchantingResult getLastClickedRecipe() {
    ...
    public void draw(PoseStack matrices, int x, int y, int mouseX, int mouseY, float delta) {
        ...
        for (EnchantingAnimatedResultButton animatedResultButton : this.resultButtons) {
